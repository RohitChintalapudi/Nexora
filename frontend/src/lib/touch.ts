...
/** Pointer capture, best effort. WebKit throws `NotFoundError` when the pointer is already gone by the time the handler runs — routine on iOS, where the system can claim the touch first — and an uncaught throw takes the rest of the handler, the gesture included, down with it. Touch pointers carry implicit capture anyway, so losing it is never fatal. */
