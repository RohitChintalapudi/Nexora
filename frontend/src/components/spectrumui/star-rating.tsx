...
export function StarRating({
  ...
  // Stored as state so the previous render's value is available during render
  const [previousPreview, setPreviousPreview] = useState(previewValue);
  if (previousPreview !== previewValue) {
    setPreviousPreview(previewValue);
  }
  const direction = previewValue >= previousPreview ? 1 : -1;
  ...
  return (
    ...
    >
      ...
      {showValue && (
        <span
          className={cn(
            "inline-flex font-medium tabular-nums text-slate-500",
            textClass,
            valueGapClass,
          )}
          aria-hidden="true"
